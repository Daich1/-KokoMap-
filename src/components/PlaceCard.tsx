      ...
      >
        <ToggleGroupItem
          ...
          className={cn(
            ...
            compact ? "text-xs h-7" : "text-xs h-8"
          )}
        >
          ⭐ 行きたい
        </ToggleGroupItem>
    ...
    <div className={containerClass}>
      {hasImage ? (
        ...
        <div className="w-full h-full flex items-center justify-center">
          <MapPin className={cn("text-muted-foreground/20", compact ? "size-8" : "size-10")} />
        </div>
      )}
...
function GridCard({ place, onSelect, distanceText }: Omit<PlaceCardProps, "compact">) {
  ...
  return (
    <div
      className="flex flex-col min-w-0 rounded-2xl bg-card shadow-sm hover:shadow-xl hover:-translate-y-0.5 transition-all duration-200 cursor-pointer overflow-hidden group"
